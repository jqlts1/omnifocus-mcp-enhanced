...
export const schema = z.object({
  hideCompleted: z
    ...
    .optional()
    .describe(
      'Set to false to show completed and dropped tasks (default: true)',
    ),
  hideRecurringDuplicates: z
    .boolean()
    .optional()
    .describe(
      'Set to true to hide duplicate instances of recurring tasks (default: true)',
    ),
});
...
Status: #next #avail #block #due #over #compl #drop\n\n`;
  ...
  const rootFolders = Object.values(database.folders).filter(
    (folder: any) => !folder.parentFolderID,
  );
  ...
  function processProject(project: any, level: number): string {
    ...
    if (
      hideCompleted &&
      (project.status === 'Done' || project.status === 'Dropped')
    ) {
      return '';
    }
    ...
    const projectTasks = database.tasks.filter(
      (task: any) => task.projectId === project.id && !task.parentId,
    );
  ...
  function processTask(task: any, level: number): string {
    ...
    if (task.childIds && task.childIds.length > 0) {
      const childTasks = database.tasks.filter((t: any) =>
  ...
  const inboxTasks = database.tasks.filter(
    (task: any) =>
      !task.projectId &&
      !task.parentId &&
      !(
