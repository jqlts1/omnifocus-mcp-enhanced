...
export const schema = z.object({
  ...
  name: z.string().optional().describe('The name of the tag to remove (as fallback if ID not provided)')
});
...
export async function handler(args: z.infer<typeof schema>, _extra: RequestHandlerExtra) {
  try {
    if (!args.id && !args.name) {
      return {
        content: [{ type: 'text' as const, text: 'Either id or name must be provided to remove a tag.' }],
        isError: true
      };
    }
    ...
    if (result.success) {
      ...
      if (childCount > 0) details.push(`⚠️ also deleted ${childCount} child tag(s)`);
    ...
    return {
      content: [{ type: 'text' as const, text: `Error removing tag: ${error.message}` }],
      isError: true
    };
  }
}
