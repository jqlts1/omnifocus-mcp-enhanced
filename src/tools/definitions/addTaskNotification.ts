...
import { addTaskNotification, formatNotification, AddNotificationParams } from '../primitives/taskNotifications.js';
...
export async function handler(args: z.infer<typeof schema>, _extra: RequestHandlerExtra) {
  try {
    ...
    if (!result.success) {
      return {
        content: [{ type: 'text' as const, text: `Failed to add notification: ${result.error}` }],
        isError: true
      };
    }
    ...
    return {
      content: [{ type: 'text' as const, text: `Error adding notification: ${error.message}` }],
      isError: true
    };
  }
}
